...
TH2F *gain2d[5];//
int number_TS_to_process=1;
int maxts(float TSvalue[10],int number_TS_to_process);
void averagePSref_phi(int runnumber,int uhtr,int refcheta=2,int refchphi=5,int whattodraw=0){
...
for(int channel=0;channel<Numberofchannels_emap;channel++){
 ...
 for(int ietan=0;ietan<10;ietan++){
   for(int phin=fibermin;phin<fibermax+1;phin++){
	 ...
	 sprintf(label,"charge eta%d phi%d TS%d led=%d",ietan,phin,TSnn,led);
	 if(gROOT->FindObject(label))  hsinglePS[ietan][phin][TSnn][led]=(TH1F*)gROOT->FindObject(label);
	 //	 else	 hsinglePS[ietan][phin][TSnn][led]= new TH1F(label,label,1000000,0,1000000);
	 else	 hsinglePS[ietan][phin][TSnn][led]= new TH1F(label,label,255,low_edge);
	 hsinglePS[ietan][phin][TSnn][led]->Reset();
	
       }
      

       sprintf(label,"avgPS_eta%d_phi%d_led=%d",ietan,phin,led);
     if(gROOT->FindObject(label))  avgPS[ietan][phin][led] = (TH1F*)gROOT->FindObject(label);
     else   avgPS[ietan][phin][led]= new TH1F(label,label,10,0,10);
     avgPS[ietan][phin][led]->Reset();

     sprintf(label,"TSmax_eta%d_phi%d_led=%d",ietan,phin,led);
     if(gROOT->FindObject(label))  TSmax[ietan][phin][led] = (TH1F*)gROOT->FindObject(label);
     else   TSmax[ietan][phin][led]= new TH1F(label,label,10,0,10);
     TSmax[ietan][phin][led]->Reset();

     sprintf(label,"pulse_eta%d_phi%d_led=%d",ietan,phin,led);
     if(gROOT->FindObject(label)) hpulse[ietan][phin][led] = (TH1F*)gROOT->FindObject(label);
     else   hpulse[ietan][phin][led]=new TH1F(label,label,1500000,0,1500000);
     hpulse[ietan][phin][led]->Reset();

     }
   
     sprintf(label,"refpulse_eta%d_phi%d(rech%d_%d)",ietan,phin,refcheta,refchphi);
     if(gROOT->FindObject(label)) hpulseref[ietan][phin] = (TH1F*)gROOT->FindObject(label);
     else   hpulseref[ietan][phin]=new TH1F(label,label,1500000,0,1500000);
     hpulseref[ietan][phin]->Reset();
   }
 }
 for (int led=0;led<2;led++){
   sprintf(label,"spectraVS_fiber_led=%d",led);
...
sprintf(label,"reflectionentriesmap");
 ...
 //read TTree
 int ieta[Numberofchannels],phi[Numberofchannels];
...
Events->GetEntry(0);
 ...
 for (Int_t i=0; i<nentries; i++) {
   ...
   for (channelN=0;channelN<Numberofchannels_emap;channelN++){
     ...
     TSmax[ch2eta[channelN]][ch2phi[channelN]][led_trigger]->Fill(maxtsn);
     ...
     for(int TS=0;TS<10;TS++){
       ...
       hsinglePS[ch2eta[channelN]][ch2phi[channelN]][TS][led_trigger]->Fill(pulse[channelN][TS]);
     ...
     if (led_trigger==0) {
       spectr[led_trigger]->Fill(ch2phi[channelN]*10+ch2eta[channelN],sumTS);
       hpulse[ch2eta[channelN]][ch2phi[channelN]][led_trigger]->Fill(sumTS);
     }
      if (led_trigger==1) {
	spectr[led_trigger]->Fill(ch2phi[channelN]*10+ch2eta[channelN],sumTS);
       hpulse[ch2eta[channelN]][ch2phi[channelN]][led_trigger]->Fill(sumTS);
 ...
 //Filling Pulse shape histo
  for(int ietan=0;ietan<6;ietan++){
    for(int phin=fibermin;phin<fibermax+1;phin++){
      for (int led=0;led<2;led++){
	for(int TSnn=0;TSnn<10;TSnn++){
	  avgPS[ietan][phin][led]->Fill(TSnn,hsinglePS[ietan][phin][TSnn][led]->GetMean());
	  avgPS[ietan][phin][led]->SetBinError(TSnn+1,hsinglePS[ietan][phin][TSnn][led]->GetRMS());
	}
	meansignalmap[led]->SetBinContent(phin-fibermin+1,ietan+1,hpulse[ietan][phin][led]->GetMean());
	rmssignalmap[led]->SetBinContent(phin-fibermin+1,ietan+1,hpulse[ietan][phin][led]->GetRMS());
      }
    }
  }

//second scan reference
  if (refchphi+refcheta!=0){
   for (Int_t i=0; i<nentries; i++) {
     if(i%(100+(900*(i>999)))==0)  printf("processing %d event\n",i);
     ...
     float refchpulse=0;
     for (channelN=0;channelN<Numberofchannels_emap;channelN++){
       //  printf("channelN %d ch2phi %d ch2eta %d\n",channelN,ch2phi[channelN],ch2eta[channelN]);
       if ((ch2phi[channelN]==refchphi)&&(ch2eta[channelN]==refcheta)){
	 // printf("channelref %d\n",channelN);
	 int maxtsn=maxts(pulse[channelN],number_TS_to_process);
	 for(int TS=0;TS<10;TS++){
	 if((TS>=maxtsn)&&(TS<maxtsn+number_TS_to_process)) refchpulse+=pulse[channelN][TS];  
       ...
       printf("refchpulse %f\n",refchpulse);
       refchpulse-=meansignalmap[0]->GetBinContent(refchphi-fibermin+1,refcheta+1);
     ...
     for (channelN=0;channelN<Numberofchannels_emap;channelN++){
       ...
       //printf("refchpulse %f\n",refchpulse);
       float normfactor=meansignalmap[1]->GetBinContent(refchphi-fibermin+1,refcheta+1);
       normfactor-= meansignalmap[0]->GetBinContent(refchphi-fibermin+1,refcheta+1);
       normfactor=1/refchpulse; //normfactor/refchpulse;
       //  printf("normfactor %f refpulse %f pedestal %f\n ",normfactor,refchpulse,meansignalmap[0]->GetBinContent(refchphi-fibermin+1,refcheta+1));
       maxtsn=maxts(pulse[channelN],number_TS_to_process);
       for(int TS=0;TS<10;TS++){
	 if((TS>=maxtsn)&&(TS<maxtsn+number_TS_to_process)) sumTS+=pulse[channelN][TS];
	 sumall+=pulse[channelN][TS];
       }
       // hpulseref[ch2eta[channelN]][ch2phi[channelN]]->Fill(normfactor*(sumTS- meansignalmap[0]->GetBinContent(ch2phi[channelN]-fibermin+1,ch2eta[channelN]+1)));
       //  if ((sumTS> (meansignalmap[0]->GetBinContent(ch2phi[channelN]-fibermin+1,ch2eta[channelN]+1)))){
	 hpulseref[ch2eta[channelN]][ch2phi[channelN]]->Fill((sumTS- meansignalmap[0]->GetBinContent(ch2phi[channelN]-fibermin+1,ch2eta[channelN]+1))/refchpulse);
	 printf(" eta %d phi %d refchpulse %f  sumTS %d pedestal %f subtract %f pedref %f\n",ch2eta[channelN],ch2phi[channelN],refchpulse,sumTS,meansignalmap[0]->GetBinContent(ch2phi[channelN]-fibermin+1,ch2eta[channelN]+1),sumTS- meansignalmap[0]->GetBinContent(ch2phi[channelN]-fibermin+1,ch2eta[channelN]+1),meansignalmap[0]->GetBinContent(refchphi-fibermin+1,refcheta+1));
	 //	 }
     }
   }
   ...
   for(int ietan=0;ietan<6;ietan++){
     for(int phin=fibermin;phin<fibermax+1;phin++){
       reflectionentriesmap->SetBinContent(phin-fibermin+1,ietan+1,(hpulseref[ietan][phin]->GetEntries()-hpulseref[ietan][phin]->GetBinContent(0))/hpulse[ietan][phin][1]->GetEntries());
       reflectionmap->SetBinContent(phin-fibermin+1,ietan+1,hpulseref[ietan][phin]->GetMean());
				    }
     }
   }
 //end second scan reference
  ...
  for(fiber=fibermin;fiber<fibermax+1;fiber++){
	...
	{  
	  ...
	  hpulse[fiberch][fiber][whattodraw-1]->GetXaxis()->SetRangeUser(0,maximumvalue-5*(maximumvalue>5));

	  while( (hpulse[fiberch][fiber][whattodraw-1]->Integral())>((hpulse[fiberch][fiber][whattodraw-1]->GetEntries())*0.05))
	    {
  ...
  for (int led=0;led<2;led++){
    for (channelN=0;channelN<Numberofchannels_emap;channelN++){
      if (maxrange[led]<hpulse[ch2eta[channelN]][ch2phi[channelN]][led]->GetMean()) 
	maxrange[led]=hpulse[ch2eta[channelN]][ch2phi[channelN]][led]->GetMean();
    }
  }
 ...
 out = fopen (name, "a");
   for(int ietan=0;ietan<6;ietan++){
     for(int phin=fibermin;phin<fibermax+1;phin++){
       fprintf(out,"%d\t%d\t%d\t%d\t%f\t%f\n",refchphi,refcheta,phin,ietan,reflectionentriesmap->GetBinContent(phin-fibermin+1,ietan+1),reflectionmap->GetBinContent(phin-fibermin+1,ietan+1));
	 }
     }
     fclose(out);*/
   //end outfile
