...
for(int channel=0;channel<Numberofchannels;channel++){
 ...
 for(int ietan=0;ietan<10;ietan++){
   for(int depthn=fibermin;depthn<fibermax+1;depthn++){
     ...
     else   hpulse[ietan][depthn][led]=new TH1F(label,label,100000,0,1000000);
 ...
 for (int led=0;led<2;led++){
   ...
   else  spectr[led] = new TH2F(label,label,10*(fibermax-fibermin+1),fibermin*10,(fibermax+1)*10,10000,0,1000000);
   spectr[led]->Reset();
