...
void FR_averagePS(int runnumber,int uhtr){
...
for(int channel=0;channel<Numberofchannels;channel++){
 ...
 for(int ietan=0;ietan<10;ietan++){
   for(int depthn=fibermin;depthn<fibermax+1;depthn++){
     ...
     else   integQ[ietan][depthn][led]= new TH1F(label,label,1000,0,10000);
 ...
 for (int i = 0; i < 10; i++)
   {
     sprintf(label, "Pulse shape in event %i", i);
     indivPulse[i] = new TH1F(label, label, 10,0,10);
   }
 ...
 pShape = new TCanvas ("Pulse shape for 10 events", "Pulse shape for 10 events", 400, 400);
 pShape->Divide(5,2);
 int index = 0;
 for (Int_t i=0; i<nentries; i++) {
   ...
   for (channelN=0;channelN<Numberofchannels;channelN++){
     ...
     // if (led_trigger==1) {
       ...
       if((index < 10) && (channelN == 1) && (led_trigger == 1))
	 {
	   for(int TS=0;TS<10;TS++){ // loop over time slices  
	     
	     indivPulse[index]->Fill(TS,pulse[channelN][TS]);	   
	   }	// loop over time slices 
	   pShape->cd(index+1);
	   indivPulse[index]->Draw();
	   index+=1;
	 }
