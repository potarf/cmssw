...
void ARJAN2_averagePS(int runnumber,int uhtr){
...
for(int channel=0;channel<Numberofchannels;channel++){
 ...
 for (int led=0;led<2;led++){
   ...
   else  spectr[led] = new TH2F(label,label,10*(fibermax-fibermin+1),fibermin*10,(fibermax+1)*10,1000,0,500000);
   spectr[led]->Reset();
 ...
 //Filling Pulse shape histo
  ...
  for(fiber=fibermin;fiber<fibermax+1;fiber++){
    for(fiberch=0;fiberch<6;fiberch++){
      ...
      int binmax = avgPS[fiberch][fiber][1]->GetMaximumBin();
      if ((binmax > 1) && (binmax < 9))
	{ cout << "Integral of fiber" << fiber << "ch" << fiberch << " = " << avgPS[fiberch][fiber][1]->Integral(binmax-1, binmax+2) << endl; }
      else if (binmax == 1)
	{ cout << "Integral of fiber" << fiber << "ch" << fiberch << " = " << avgPS[fiberch][fiber][1]->Integral(binmax, binmax+2) << endl; }
      else
	{ cout << "Integral of fiber" << fiber << "ch" << fiberch << " = ? " << endl; }
