...
void tree_analyze_all(int runnumber,int uhtr,char *filename="test.root"){
   ReadBVmap();
...
for(int channel=0;channel<Numberofchannels_emap;channel++){
 ...
 ///create histos
   ...
   for(int iFIBERCHn=0;iFIBERCHn<6;iFIBERCHn++){
	 ...
	 sprintf(label,"avgPS_rbx%d_fiberch%d_fiber%d_led=%d",rbxnumber,iFIBERCHn,FIBERn,led);
	 ...
	 sprintf(label,"TSmax_rbx%d_fiberch%d_fiber%d_led=%d",rbxnumber,iFIBERCHn,FIBERn,led);
	 ...
	 sprintf(label,"pulse_rbx%d_fiberch%d_fiber%d_led=%d",rbxnumber,iFIBERCHn,FIBERn,led);
	 if(gROOT->FindObject(label)) hpulse[iFIBERCHn][FIBERn][led] = (TH1F*)gROOT->FindObject(label);
	 else   hpulse[iFIBERCHn][FIBERn][led]=new TH1F(label,label,1000000/4,0,1000000);
 ...
 printf("next step1\n");
   ...
   for (int led=0;led<2;led++){
     ...
     else  spectr[led] = new TH2F(label,label,10*(fibermax-fibermin+1),fibermin*10,(fibermax+1)*10,1000000/4,0,1000000);
     spectr[led]->Reset();
